...
(define (kv-cas k from to #:create-if-missing? [create-if-missing #f])
  ...
  (case (message-type resp)
    ...
    [("error") (case (message-ref resp 'code)
                 [(22) (begin
                         (log-counter-debug "Failing cas ~v ~v ~v because: ~v" k from to (message-ref resp 'text))
                         #f)]
...
(add-handler node
             ...
             (lambda (req)
               ...
               (kv-write (node-key) (+ current (message-ref req 'delta)))
               (respond req)))

(add-handler node
             "read"
             (lambda (req)
               (respond req (hash 'value (kv-read (node-key))))))
